...
function createSubmittedClaim(createSubmittedClaim) {
    ...
    //var val = submittedDentalClaim.getDecorator('description').getArguments()[1];
...
function getSubmittedClaim(getSubmittedClaim) {
    ...
    return getAssetRegistry(NS + '.SubmittedDentalClaim')
        .then(function (submittedClaimAssetRegistry) {
            return submittedClaimAssetRegistry.get("1");
        })
        ...
        .then(function (submittedDentalServiceLineRegistry) {
            console.log("The service line: " + theSubmittedDentalServiceLines[0]);
            return submittedDentalServiceLineRegistry.get(theSubmittedDentalServiceLines[0].$identifier);
        })
        .then(function (submittedDentalServiceLine) {
            console.log(JSON.stringify(getSerializer().toJSON(submittedDentalServiceLine)));
        })
        .catch(function (error) {
            console.log('Error: ' + error);
        });
