...
function createBillingProvider(billingProvider) {
    ...
    var aProviderName = theFactory.newConcept(NS, 'HumanName');
...
function createSubscriber(subscriber) {
    ...
    // Name
    var theSubscriberName = theFactory.newConcept(NS, 'HumanName');
    ...
    // Comms
    theSubscriber.electronicAddresses = [];
    var theElectronicAddress = theFactory.newConcept(NS, 'ElectronicAddress');
    theElectronicAddress.electronicAddressType = 'EMAIL';
    theElectronicAddress.emailAddress = 'ajb@blakey.org';
    theSubscriber.electronicAddresses.push(theElectronicAddress);
    // Phone
...
function adjudicateDentalClaim(adjudicateDentalClaim) {
    ...
    // Create an adjudicated claim that references the submitted cliam - lame PoC

    var NS = 'org.acme.claim';
    var theFactory = getFactory();

    //console.log('The claim: ' + JSON.stringify(getSerializer().toJSON(adjudicateDentalClaim))); 
    ...
    theAdjudicatedDentalClaim.submittedDentalClaim = theFactory.newRelationship(NS, 'SubmittedDentalClaim', '1');
    //    adjudicateDentalClaim.submittedDentalClaim.dentalClaimId);
    return getAssetRegistry(NS + '.AdjudicatedDentalClaim')
        .then(function (adjudicatedDentalClaimAssetRegistry) {
            console.log('Add the new adjudicated dental claim: ' + JSON.stringify(getSerializer().toJSON(theAdjudicatedDentalClaim))); 
 ...
 * @param {org.acme.claim.AddMembers} addMembers - the transaction to be processed
 * @transaction
 * @return {Promise} a promise that resolves when transaction processing is complete
 */
function addMembers(addMembers) {
    // 
    var theMemberHeader = memberAdditions.header; // We are going to enroll everyone in the same plan?
    // From the header 
